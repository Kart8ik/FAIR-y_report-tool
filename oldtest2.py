...
def requirement_popup():
    ...
    popup.title("FAIR Requirement")
    popup.grab_set()

    result = {"ok": False}

    tk.Label(popup, text="Requirement").grid(row=0, column=0, padx=8, pady=5, sticky="e")
    tk.Label(popup, text="- Tol").grid(row=1, column=0, padx=8, pady=5, sticky="e")
    ...
    neg.grid(row=1, column=1, sticky="w")
    pos.grid(row=2, column=1, sticky="w")
...
def add_bubble(event):
    ...
    # 👉 ADD BUBBLE FIRST (IMMEDIATE VISUAL FEEDBACK)
    bubbles.append({
    ...
    render()  # <-- bubble appears immediately
    ...
    if data.get("ok"):
        ...
        bubbles.pop()  # remove bubble if cancelled
...
def update_bubble_list():
    ...
    bubble_listbox.insert(tk.END, "No | Req   | -Tol | +Tol")
    bubble_listbox.insert(tk.END, "-" * 32)
...
def save_report():
    ...
    report_file = f"FAIR_Report_{timestamp}.xlsx"
    ...
    def copy_row_style(src_row, dest_row):
    ...
    if bubble_count > 1:
        ws.insert_rows(START_ROW + 1, amount=bubble_count - 1)
        for i in range(1, bubble_count):
            target_row = START_ROW + i
            copy_row_style(template_row, target_row)
            for merged in list(ws.merged_cells.ranges):
                if merged.min_row == START_ROW and merged.max_row == START_ROW:
                    ws.merge_cells(
